# IDEA: Unresolved forward action from TASK-931
...
**TASK-931** closed with Forward Action:

> `deterministic-hansei-checker` can now reliably use `task.lockedCommit` as a diff baseline when tasks are reloaded across sessions.
