...
- [x] `arch task start TASK-XXX` emits a `## Constraint Preflight` block to stdout listing: ADRs whose file references overlap the task's Context paths, open tensions whose affected tasks or file scope overlaps, and recent (≤30d) Hansei signals by category for the context paths.  →  prose: verified by running arch task start on a task with known overlapping ADRs and confirming preflight output
- [x] If no constraints are found for any section, that section is omitted entirely (no "none found" noise).  →  prose: verified by unit test fixture with non-overlapping context path; sections omitted individually confirmed
- [x] Preflight completes in < 500ms on a repo with 50+ ADRs.  →  prose: 242ms wall time on 21 ADRs, 5 tensions, ~940 archive tasks
- [x] Preflight output goes to stdout only. No writes to INBOX, escalations.jsonl, or the task file.  →  grep: no appendFile or writeFile calls in constraint-preflight.ts
- [x] `arch review` passes.  →  cmd: bash scripts/arch.sh review; exit: 0
- [x] CLI tests pass.  →  cmd: npm test --prefix cli; exit: 0

### Definition of Done

- [x] An agent running `arch task start` on a task that touches files covered by an ADR sees that ADR's constraint before implementation begins.
- [x] The preflight is silent when there are no relevant constraints (zero noise overhead on unrelated tasks).
- [x] `arch review` passes.  →  cmd: bash scripts/arch.sh review; exit: 0
