...
**Closed-at:** 2026-05-26T15:40:00Z
**Actor:** unknown
...
**Date:** 2026-05-26
**AC1** (version written correctly): PASS — manual: `arch.config.json` shows `"version": "1.2.0"`. AC cmd path `smartcart-os/arch.config.json` is relative to wrong root — AC authoring error, not implementation failure.
**AC2** (npm test): PASS — 647/647 pass. Missing `cd cli &&` prefix caused verifier failure.
**AC3** (arch review): PASS — exits 0 manually. Verifier ran while task was in REVIEW queue, causing false exit 1.
**Hansei:** Accepted. H0 with specific file/method references. 4-line fix, no deviation from plan.