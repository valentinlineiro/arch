...
3. **Resurrection Review (every 20 govern ticks):** Count TTL-rejected and DEFERRED IDEAs in `docs/refinement/archive/`. If count > 10 OR READY task count < 3, emit `[RESURRECTION-REVIEW] N TTL-rejected/DEFERRED IDEAs in archive — run arch inbox --resurrect to evaluate` to `docs/INBOX.md`. Do not auto-resurrect. Human decides.
4. **Immediate Improvements:** Identify context gaps or guideline changes based on patterns observed across phases.
5. **Sprint Metrics:** Run `arch report` to update `docs/METRICS.md` with the latest operational indicators.
