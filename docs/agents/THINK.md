...
3. **Replenishment check:** Count `READY` tasks in `docs/tasks/`. If count < 3, propose at least one new IDEA in `docs/refinement/` before continuing. This threshold is a hard rule (core.md §5) — not optional. **Replenishment cap:** Do not propose more than 3 new IDEAs in a single THINK session regardless of how low the READY count is. If READY < 3 and the refinement queue already has ≥ 5 unresolved DRAFT IDEAs, skip replenishment — the queue is the bottleneck, not the idea count.
