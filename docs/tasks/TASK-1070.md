...
**Meta:** P3 | XS | IN_PROGRESS | Focus:no | 2-code-generation | local | cli/src/test/ts/
**Actor:** unknown
...
- [ ] The four vacuous assertions are replaced with deterministic assertions (`assert.strictEqual` or equivalent)  →  file: cli/src/test/ts/meta-line-parser.test.ts

- [ ] No test in the file uses `assert.ok(task === null || task)` after the fix  →  cmd: grep -cn "task === null || task" cli/src/test/ts/meta-line-parser.test.ts; exit: 1
