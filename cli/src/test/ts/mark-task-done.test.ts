...
import { Task, TaskStatus, FocusLevel, Hansei } from '../../main/ts/domain/models/task.js';
import { MockFileSystem, MockTaskRepository, MockGitRepository, MockReviewer, MockCausalSignalLog } from './mocks/index.js';

const validHansei: Hansei = {
  severity: 'H1',
  category: '[TypeHack]',
...
function makeTask(overrides: Partial<Task> = {}): Task {
  const base: Task = {
    ...
    context: ['src/'],
    content: '',
    filePath: 'docs/tasks/TASK-031.md',
    acceptanceCriteria: [],
...
function makeMockReviewer(valid = true): MockReviewer {
  const reviewer = new MockReviewer();
  if (!valid) reviewer.violations = ['Invalid format'];
  return reviewer;
}
...
test('MarkTaskDone - sets status to DONE', async () => {
  ...
  repo.tasks = [task];
  const useCase = new MarkTaskDone(repo, makeMockReviewer(), makeFs());

  await useCase.execute('TASK-031');

  const saved = repo.tasks.find(t => t.id === 'TASK-031');
  assert.strictEqual(saved?.status, TaskStatus.DONE);
});
...
test('MarkTaskDone - blocks transition when pending ACs exist', async () => {
  ...
  reviewer.violations = ['Pending ACs']; // Force failure
  const useCase = new MarkTaskDone(repo, reviewer as any, makeFs());
...
test('MarkTaskDone - injects closedAt timestamp on DONE transition', async () => {
  ...
  const saved = repo.tasks.find(t => t.id === 'TASK-031');
  assert.ok(saved?.closedAt, 'closedAt should be set');
  assert.ok(!isNaN(Date.parse(saved!.closedAt!)), 'closedAt should be a valid ISO date');
});
...
test('MarkTaskDone - does not overwrite existing closedAt (idempotent)', async () => {
  ...
  const saved = repo.tasks.find(t => t.id === 'TASK-031');
  assert.strictEqual(saved?.closedAt, existing);
});
...
test('MarkTaskDone - force path also injects closedAt', async () => {
  ...
  const saved = repo.tasks.find(t => t.id === 'TASK-031');
  assert.ok(saved?.closedAt, 'closedAt should be set even on force');
});
...
test('MarkTaskDone - XS post-rollout task without Hansei section proceeds without error', async () => {
  ...
  const saved = repo.tasks.find(t => t.id === 'TASK-195');
  assert.strictEqual(saved?.status, TaskStatus.DONE);
});
...
test('MarkTaskDone - allows pre-rollout task without Hansei section', async () => {
  ...
  const saved = repo.tasks.find(t => t.id === 'TASK-194');
  assert.strictEqual(saved?.status, TaskStatus.DONE);
});
...
function makeMockFsWithInbox(config?: object): MockFileSystem {
  const fs = new MockFileSystem();
...
test('L3 gate — XS task with passing cmd AC qualifies for self-archive', async () => {
  const repo = new MockTaskRepository();
  repo.tasks = [XS_TASK_WITH_CMD_AC];
  const fs = makeMockFsWithInbox();
  const useCase = new MarkTaskDone(repo, makeMockReviewer() as any, fs);

  await useCase.execute('TASK-200');
  const saved = repo.tasks.find(t => t.id === 'TASK-200');
  assert.equal(saved?.status, TaskStatus.DONE, 'task should be DONE');
...
test('L3 gate — M task fails gate, no L3-AUTO in INBOX', async () => {
  const repo = new MockTaskRepository();
  repo.tasks = [M_TASK_WITH_CMD_AC];
  const fs = makeMockFsWithInbox();
  const useCase = new MarkTaskDone(repo, makeMockReviewer() as any, fs);

  await useCase.execute('TASK-201');
  const saved = repo.tasks.find(t => t.id === 'TASK-201');
  assert.equal(saved?.status, TaskStatus.DONE);
  const inbox = fs.files['docs/INBOX.md'] ?? '';
...
test('L3 gate — S prose-only task falls back to human review (no L3-AUTO)', async () => {
  const repo = new MockTaskRepository();
  repo.tasks = [S_TASK_PROSE_ONLY];
  const fs = makeMockFsWithInbox();
  const useCase = new MarkTaskDone(repo, makeMockReviewer() as any, fs);

  await useCase.execute('TASK-203');
  const saved = repo.tasks.find(t => t.id === 'TASK-203');
  assert.equal(saved?.status, TaskStatus.DONE);
  const inbox = fs.files['docs/INBOX.md'] ?? '';
...
test('MarkTaskDone — computes turn count from git log when lockedCommit present', async () => {
  ...
  git.getCommitCountBetween = async () => 7;

  const useCase = new MarkTaskDone(repo, reviewer as any, fs, undefined, undefined, undefined, undefined, git);
  await useCase.execute('TASK-500');

  const saved = repo.tasks.find(t => t.id === 'TASK-500');
  assert.equal(saved?.turns, 7, 'turns should be 7 (mocked git count)');
});
...
test('MarkTaskDone — turns is null when lockedCommit absent', async () => {
  ...
  const reviewer = makeMockReviewer();
  const git = new MockGitRepository();
  git.getCommitCountBetween = async () => 5;

  const useCase = new MarkTaskDone(repo, reviewer as any, fs, undefined, undefined, undefined, undefined, git);
  await useCase.execute('TASK-501');

  const saved = repo.tasks.find(t => t.id === 'TASK-501');
  assert.equal(saved?.turns ?? null, null, 'turns should be null when no lockedCommit');
});
...
test('MarkTaskDone — auto-emits implements signal for explicit ADR reference in content', async () => {
  ...
  const useCase = new MarkTaskDone(repo, makeMockReviewer() as any, fs, undefined, undefined, causalLog as any);
  await useCase.execute('TASK-600');

  const implementsSignals = causalLog.emitted.filter((s: any) => s.candidate_relation === 'implements');
...
test('MarkTaskDone — auto-emits caused_by signal for each TASK-ID in depends', async () => {
  ...
  const causedBySignals = causalLog.emitted.filter((s: any) => s.candidate_relation === 'caused_by');
...
test('MarkTaskDone — no signals emitted for empty content and no depends', async () => {
  ...
  assert.equal(causalLog.emitted.length, 0, `should emit no signals, got: ${JSON.stringify(causalLog.emitted)}`);
});
...
test('L3 gate — M task in 6-writing with all cmd: ACs qualifies for self-archive', async () => {
  const repo = new MockTaskRepository();
  repo.tasks = [M_6WRITING_ALL_CMD];
  const fs = makeMockFsWithInbox({ hanseiSinceTaskId: 1, governance: { protectedPaths: ['docs/adr/'] } });
  const git = new MockGitRepository();
  git.changedFilesInLastCommit = [];
  const useCase = new MarkTaskDone(repo, makeMockReviewer() as any, fs, undefined, undefined, undefined, undefined, git);

  await useCase.execute('TASK-210');
  const saved = repo.tasks.find(t => t.id === 'TASK-210');
  assert.equal(saved?.status, TaskStatus.DONE);
  const inbox = fs.files['docs/INBOX.md'] ?? '';
...
test('L3 gate — M task in 6-writing with prose AC is NOT L3 eligible', async () => {
  const repo = new MockTaskRepository();
  repo.tasks = [M_7OPS_PROSE_AC];
  const fs = makeMockFsWithInbox({ hanseiSinceTaskId: 1, governance: { protectedPaths: [] } });
  const git = new MockGitRepository();
  const useCase = new MarkTaskDone(repo, makeMockReviewer() as any, fs, undefined, undefined, undefined, undefined, git);

  await useCase.execute('TASK-211');
  const saved = repo.tasks.find(t => t.id === 'TASK-211');
  assert.equal(saved?.status, TaskStatus.DONE);
  const inbox = fs.files['docs/INBOX.md'] ?? '';
...
test('L3 gate — M task in 6-writing with protected path modified is NOT L3 eligible', async () => {
  const repo = new MockTaskRepository();
  repo.tasks = [M_6WRITING_ALL_CMD];
  const fs = makeMockFsWithInbox({ hanseiSinceTaskId: 1, governance: { protectedPaths: ['cli/src/main/ts/domain/models/'] } });
  const git = new MockGitRepository();
  git.changedFilesInLastCommit = ['cli/src/main/ts/domain/models/task.ts'];
  const useCase = new MarkTaskDone(repo, makeMockReviewer() as any, fs, undefined, undefined, undefined, undefined, git);
