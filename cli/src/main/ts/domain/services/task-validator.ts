...
export class TaskValidator {
  ...
  private static META_LINE_REGEX = /^\*\*Meta:\*\* P(?<priority>[0-3]) \| (?<size>XS|S|M|L|XL) \| (?<status>IDEA|BACKLOG|READY|IN_PROGRESS|REVIEW|DONE|BLOCKED|REJECTED) \| (?<focus>Focus:yes|Focus:no) \| (?<class>\d-[a-z-]+) \| (?<cli>[a-z-]+) \| (?<context>.+)$/;
